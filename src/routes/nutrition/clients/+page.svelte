<script lang="ts">
    ...
    import { Undo2, User, Mail, Phone, Loader2, Apple, ChevronDown, ChevronUp, Calendar, DollarSign, Package } from "lucide-svelte";
    import { user } from "$lib/stores/auth";
    import { getAllMembers } from "$lib/api/users";
    import { apiGet } from "$lib/api/config";
    import TableToolbar from "$lib/TableToolbar.svelte";
    import { nutritionPlans } from "$lib/data/categories";
    ...
    onMount(async () => {
        try {
            ...
            console.log('=== NUTRITION PAGE DEBUG ===');
            ...
            if ($user && $user.role === 'admin') {
                ...
                // Filter orders by service_type='nutrition' and status='paid'
                const nutritionOrders = allOrders.filter(order =>
                    order.service_type === 'nutrition' && order.status === 'paid'
                );

                console.log('Nutrition orders (filtered by service_type):', nutritionOrders.length);
                console.log('Nutrition orders breakdown by staff_id:', {
                    assigned: nutritionOrders.filter(o => o.staff_id !== null).length,
                    unassigned: nutritionOrders.filter(o => o.staff_id === null).length
                });

                // Get unique user_ids from nutrition orders
                const nutritionUserIds = [...new Set(nutritionOrders.map(order => order.user_id))];

                console.log('Unique nutrition user IDs:', nutritionUserIds);

                // Filter members to only those with nutrition orders
                clients = allMembers.filter(member => nutritionUserIds.includes(member.id));

                console.log('Filtered nutrition clients:', clients.length);

            } else if ($user && $user.role === 'nutrition') {
                console.log('Nutritionist user detected - fetching assigned clients only (filtered by staff_id)');
                ...
                console.log('Nutritionist ID:', $user.id);

                // Filter orders by:
                // 1. service_type='nutrition'
                // 2. status='paid'
                // 3. staff_id matches current nutritionist
                const myNutritionOrders = allOrders.filter(order =>
                    order.service_type === 'nutrition' &&
                    order.status === 'paid' &&
                    order.staff_id === $user.id
                );

                console.log('My nutrition orders (filtered by service_type + staff_id):', myNutritionOrders.length);

                // Get unique user_ids from my nutrition orders
                const myClientIds = [...new Set(myNutritionOrders.map(order => order.user_id))];
                ...
                console.log('My nutrition clients:', clients.length);
            } else {
                // Unauthorized role
                error = 'Access denied. Only admins and nutritionists can access this page.';
            }
        } catch (err) {
            console.error("Error fetching nutrition clients:", err);
    ...
    function viewClientProfile(clientId: number) {
        window.location.href = `/nutrition/clients/${clientId}`;
    }
    ...
    function applyFilters() {
        ...
        // Service type filter (nutrition plan)
        if (currentFilters.serviceType) {
            result = result.filter(client =>
                client.nutrition_plan === currentFilters.serviceType
            );
        }
...
>
    ...
    <div class="text-center mb-12 max-w-4xl">
        <h1 class="text-4xl sm:text-5xl font-extrabold leading-tight">
            {#if $user?.role === 'admin'}
                All <span class="text-[#F41A53]">Nutrition Clients</span>
            {:else}
                My <span class="text-[#F41A53]">Nutrition Clients</span>
            {/if}
        </h1>
        <p class="mt-4 text-lg sm:text-xl text-gray-300">
            {#if $user?.role === 'admin'}
                View and manage all nutrition clients across the system.
            {:else}
                View and manage your assigned nutrition clients.
            {/if}
        </p>
    </div>
    ...
    <div class="w-full max-w-7xl">
        ...
        {:else if clients.length === 0}
            <div class="bg-gray-800 bg-opacity-50 border border-gray-700 rounded-lg p-12 text-center">
                <Apple size="64" class="mx-auto mb-4 text-gray-500" />
                <p class="text-xl text-gray-300 mb-2">No Nutrition Clients Yet</p>
                <p class="text-gray-400">
                    {#if $user?.role === 'admin'}
                        No clients have purchased nutrition services yet.
                    {:else}
        ...
        {:else}
            ...
            <TableToolbar
                ...
                serviceTypeOptions={nutritionPlans}
                    ...
                    <table class="w-full">
                        <thead class="bg-[#F41A53]">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-sm font-semibold">Nutrition Plan</th>
                        ...
                        <tbody class="divide-y divide-gray-700">
                            {#each filteredClients as client (client.id)}
                                ...
                                <tr class="hover:bg-gray-800 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2 text-gray-300">
                                            <Apple size="16" class="text-gray-500" />
                                            <span class="truncate max-w-xs">
                                                {client.nutrition_plan || "No plan assigned"}
                                            </span>
                                        </div>
                                    </td>
                                                        ...
                                                        <div class="order-item">
                                                            <div class="flex items-center justify-between">
                                                                <div class="flex items-center gap-3">
                                                                    ...
                                                                    <span class="text-gray-300">{order.title || 'Nutrition Plan'}</span>
                                                                </div>
